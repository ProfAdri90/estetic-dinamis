...
export default function TeamCard({ name, title, image }) {
  return (
  <div className="relative group w-full h-full overflow-hidden rounded-xl shadow-md">
  <Image
    src={image}
    alt={name}
    width={300}
    height={200}
    className="object-cover w-full h-[180px] filter grayscale group-hover:grayscale-0 transition-all duration-500 ease-in-out"
  />
 <div className="w-[12.5%] px-1">
  <div className="relative group w-full aspect-[3/4] overflow-hidden rounded-xl shadow-md">
    <Image
      src={image}
      alt={name}
      fill
